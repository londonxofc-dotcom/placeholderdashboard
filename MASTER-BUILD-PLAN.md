...
**Progress:** Phase 0: 100% | Phase 1: 100% | Phase 2: 100% | Phase 3: 100% — Jarvis ✅ Wakanda ✅ Cockpit mode switcher ✅ Cockpit page tests ✅. Phase 4 service contracts are partially landed: memory isolation service ✅, executor isolation write path ✅, graph/tool-wrapper isolation write paths ✅, role registry ✅, ABAC role-layer tests ✅, Batman actor-role propagation ✅, tool/role vocabulary alignment ✅, Batman multi-approver chain ✅, Resonance OS scoping ✅. Phase 5 operator guide ✅, README refresh ✅, deployment guide ✅, deployment decision record ✅, monitoring runbook ✅, cockpit idle-polling optimization ✅, backend readiness probe ✅, required credential readiness check ✅, placeholder credential readiness guard ✅, local deployment preflight ✅, launch-gate decision preflight ✅, launch-gate production env preflight ✅, CORS origin normalization ✅, runtime wildcard CORS guard ✅, protected-file preflight guard ✅, cockpit summary-based observability ✅, runtime status endpoint ✅, runtime probe discovery ✅, pre-launch degraded-readiness probe mode ✅, probe base-url `/api` guard ✅, probe JSON status guard ✅, probe verdict/policy alignment ✅, non-secret env readiness check ✅, production wildcard CORS env guard ✅, production exact-origin env guard ✅, production frontend API URL guard ✅, deployment decision checker ✅, deployment probe checker ✅, backend task lookup optimization ✅, supervisor task lookup optimization ✅.
...
- [x] Probe checker rejects backend base URLs that include `/api` — `tools/deployment_probe_check.py`
...
- [x] Production env checker requires `NEXT_PUBLIC_API_URL` to point at `/api` — `tools/deployment_env_check.py --production`
