...
The environment check reports variable names and readiness only; it does not print secret values, rejects malformed local or production `DATABASE_URL` values, and validates configured `NEXT_PUBLIC_API_URL` and `ALLOWED_ORIGINS` entries as exact local/prod-safe URL shapes, while still allowing local wildcard CORS. Runtime CORS parsing also ignores wildcard or malformed origin entries. The decision check fails while the deployment decision record still contains `TBD` values and reports only unresolved line numbers/sections. The preflight runner can include the probe check for a running backend, and can optionally restore generated `ui/next-env.d.ts` churn after verification. The probe checker expects an HTTP(S) backend base URL without `/api`, fails fast if the URL is malformed or `/api` is included, and verifies expected JSON status values; use `--allow-degraded-ready` only for pre-launch reachability checks.
